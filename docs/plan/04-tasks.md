...
- [x] プロパティ型変更時の値マイグレーション(4-B-6、`coerce_value` で全行を新型へ移行。
      移行不能は空値へ。プロパティ削除時は孤児キーを一括除去。API で型変更対応)
...
- [x] ブロックテキストの Yjs ドキュメント化(4-D-1、サーバ側 `pages/crdt_store.py`。
      ブロックごとの `BlockDoc` ライブ状態を Django cache に保持、サーバが唯一の種まき手)
- [x] デバウンス + 永続化(4-D-1、`maybe_flush` の時間スロットリングで `Block.text` へ
      書き戻し + version 更新。切断時に最終状態を確実にフラッシュ)
- [x] Channels プロトコル(4-D-1、`crdt_sync`=state vector 差分同期 / `crdt_update`=
      マージ + 購読者へ中継。editor 以上のみ更新可・viewer は同期=読み取りのみ。
      WebSocket 越しに 2 クライアントが収束し永続化することをテストで固定)
- [ ] ブラウザ Yjs バインディング(4-D-2、contenteditable ↔ Y.Text。E2E が WS 非対応の
      ため手動検証。既存エディタを壊さないよう加算的に実装する)
- [ ] IME 変換中のリモート更新適用制御(4-D-2、既存 `isComposing` 拡張)
- [ ] リモートカーソル表示(4-D-2)
