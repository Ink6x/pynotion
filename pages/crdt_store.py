...
def sync_update(block_id, seed_text: str, client_state_b64: str | None) -> str:
    ...
    client_state = base64.b64decode(client_state_b64) if client_state_b64 else None
    return base64.b64encode(doc.update_since(client_state)).decode()


def apply_update(block_id, seed_text: str, update_b64: str) -> str:
    """クライアントの更新(base64)をライブ状態へマージし、投影テキストを返す。"""
    doc = _load_doc(block_id, seed_text)
    doc.apply_update(base64.b64decode(update_b64))
    cache.set(_state_key(block_id), doc.update_since(), CRDT_TTL)
    return doc.text
...
def flush(block_id) -> bool:
    ...
    書き戻したら True。テキスト編集なので version を進め、REST 読み取りと整合させる。
    """
    ...
    block.text = text
    block.version += 1
    block.save(update_fields=["text", "version", "updated_at"])
    return True
