...
class TestPage:
    def test_create_root_page(self, user) -> None:
        page = Page.objects.create_page(owner=user, title="はじめてのページ")
        assert page.title == "はじめてのページ"
        assert page.parent is None
        assert page.position
        assert page.is_deleted is False

    def test_create_child_page(self, user) -> None:
    ...
    def test_sibling_pages_ordered_by_position(self, user) -> None:
        first = Page.objects.create_page(owner=user, title="1")
        second = Page.objects.create_page(owner=user, title="2")
        third = Page.objects.create_page(owner=user, title="3")
        roots = list(Page.objects.alive().roots())
        assert roots == [first, second, third]

    def test_soft_delete_moves_to_trash(self, user) -> None:
        page = Page.objects.create_page(owner=user, title="削除対象")
        page.soft_delete()
        assert Page.objects.alive().count() == 0
        assert Page.objects.trashed().count() == 1
        assert page.deleted_at is not None

    def test_soft_delete_cascades_to_descendants(self, user) -> None:
        parent = Page.objects.create_page(owner=user, title="親")
        child = Page.objects.create_page(owner=user, title="子", parent=parent)
        grandchild = Page.objects.create_page(owner=user, title="孫", parent=child)
        parent.soft_delete()
        for p in (parent, child, grandchild):
            p.refresh_from_db()
            assert p.is_deleted is True

    def test_restore_revives_page_and_descendants(self, user) -> None:
        parent = Page.objects.create_page(owner=user, title="親")
        child = Page.objects.create_page(owner=user, title="子", parent=parent)
        parent.soft_delete()
        parent.restore()
    ...
    def test_soft_delete_preserves_already_trashed_child_timestamp(self, user) -> None:
        parent = Page.objects.create_page(owner=user, title="親")
        child = Page.objects.create_page(owner=user, title="子", parent=parent)
        child.soft_delete()
    ...
    def test_restore_keeps_independently_trashed_children(self, user) -> None:
    ...
    def test_restore_child_of_trashed_parent_moves_to_root(self, user) -> None:
    ...
    def test_new_page_gets_default_paragraph_block(self, user) -> None:
        page = Page.objects.create_page(owner=user, title="新規")
        assert page.blocks.count() == 1
        assert page.blocks.first().type == BlockType.PARAGRAPH


class TestBlock:
    def test_create_blocks_in_order(self, user) -> None:
    ...
    def test_insert_block_between(self, user) -> None:
        page = Page.objects.create_page(owner=user, title="p")
        page.blocks.all().delete()
    ...
    def test_todo_block_checked_state(self, user) -> None:
        page = Page.objects.create_page(owner=user, title="p")
